...
st.markdown('Enter data as numbers with decimals, separated by spaces or commas. *e.g.*:')
st.text('10, 9, 8, 7')
...
lengthOption=st.sidebar.radio("Height units", ('centimeters','inches'))
timePointsText = st.text_input("Enter list of times")
timePointsTextSplit = timePointsText.replace(',',' ').split()
timeOption=st.sidebar.radio("Time units", ("seconds","minutes"))

animateOption=st.sidebar.radio("Would you like to animate data processing?",("yes","no"))
...
if numH > 1 and numH == numT and numS == 1:
    ...
    avVel=[(waterHeightSI[i]-waterHeightSI[i+1])/(timePointsSI[i+1]-timePointsSI[i]) for i in range(numH-1)]
    ...
    if not animateLog:
        ax0.line, = ax0.plot(timePoints,waterHeight,'o')
        ax1.line, = ax1.plot(avVel, avHeight,'o')
